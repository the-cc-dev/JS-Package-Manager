 ...
 *   void     events.bind ( object target, string event, function func )
 *   void     events.unbind ( object target, string event )
 *   boolean  events.specialEvents.exists ( string name )
 *   void     events.specialEvents.add ( string name, object info )
 *   void     events.specialEvents.edit ( string name, object info )
 *   void     events.specialEvents.del ( string name )
 ...
 *
 * Constructors:
 *   events.EventsError ([ string message ]) <extends> Error
 */
...
$('events', {
	package: function(pkg) {
		...
		var win = window;
		var doc = document;
		var __version = '0.2.2-beta-pkg';
		
		var
		handlers = [ ],
		
		/**
		 * Checks for DOM loaded-ness using doc.readyState and win.onload
		 *
		 * @access  private
		 * @return  boolean
		 */
		domLoaded = (function() {
			var flag = (doc.readyState === 'complete');
			if (! flag) {
	...
	//  Build the onhashchange fix
		...
		HashChangeFix = (new (function() {
			...
			buildEventObject = function(old, info) {
				return pkg.buildEventObject('hashchange', { }, merge({
					oldURL: old,
					newURL: location + ''
				}, (info || { })));
			},
			...
			poller = (function() {
				...
				return {
					start: function() {
						if (! pollerStarted) {
							pollerStarted = true;
							pollerInterval = win.setInterval(function() {
								var current = fetch();
								if (current !== lastValue) {
									lastValue = current;
									if (! supportsOnhashchange()) {
										self.dispatchEvent(lastLocation);
									}
									lastLocation = location + '';
								}
							}, pollerRate);
					...
					stop: function() {
						if (pollerStarted) {
							pollerStarted = false;
							win.clearInterval(pollerInterval);
						}
					}
				};
			}()),
			...
			self.init = function() {
				// Attach the listener
				attachListener(win, 'hashchange', eventFunction);
			 ...
			 */
			self.addEventListener = function(func) {
				callStack.push(func);
			};
			 ...
			 */
			self.removeEventListener = function(func) {
				var newStack = [ ];
				for (var i = 0, c = callStack.length; i < c; i++) {
					if (callStack[i] !== func) {
			 ...
			 * Invoke the event
			 */
			self.dispatchEvent = function(info) {
	...
	// ----------------------------------------------------------------------------
		...
		EventController = function(event, target) {
			 ...
			 * Event properties
			 */
			self.target = self.srcElement = target;
			for (var i in event) {
			 ...
			 * @return  void
			 */
			self._setNamespace = function(ns) {
				namespace = ns;
			};
			...
			self.mousePosition = function() {
				...
				if (event.pageX || event.pageY) {
					...
					posx = event.clientX + doc.body.scrollLeft
						+ doc.docElement.scrollLeft;
					posy = event.clientY + doc.body.scrollTop
						+ doc.docElement.scrollTop;
				}
			 ...
			 * @return  boolean
			 */
			self.isDefaultCanceled = function() {
				return isCanceled;
			};
	...
	// ----------------------------------------------------------------------------
		...
		EventWrapper = function(target, event) {
			 ...
			 * @return  void
			 */
			self.registerFunction = function(func, namespaces) {
				var namespace = getNamespace(namespaces);
				namespace['.'].push(new EventFunction(func, self));
			};
			 ...
			 * @return  void
			 */
			self.removeNamespace = function(namespaces) {
			 ...
			 * @return  boolean
			 */
			self.run = function(evt, controller) {
	...
	// ----------------------------------------------------------------------------
		...
		EventHandler = function(target) {
			 ...
			 * @return  void
			 */
			self.registerEvent = function(event, func) {
	...
	//  Helper functions
		...
		attachListener = function(target, event, func) {
			if (target.addEventListener) {
				...
				throw new EventsError('Could not attach event listener');
			}
		},
		...
		detachListener = function(target, event, func) {
			if (target.removeEventListener) {
				...
				throw new EventsError('Could not detach event listener');
			}
		},
		...
		invokeListener = function(target, event, info) {
			...
			if (target.despatchEvent) {
				...
				evt = pkg.buildEventObject(target, event, info);
				...
				throw new EventsError('Could not invoke event listener');
			}
		},
	...
	//  External functions
		...
		pkg.version = function() {
			return __version;
		};
		
		pkg.EventsError = $.defineError('EventsError');
			...
			getLogger = function() {
				if (logger == null) {
					if (typeof win.console !== 'undefined') {
						if (typeof win.console.log.apply === 'function') {
							logger = function() {
		...
		pkg.invoke = function(target, event, info) {
			return invokeListener(target, event, info);
		};
	
		pkg.buildEventObject = (function() {
			...
			var eventTypes = {
				...
				MutationEvents: ['DOMAttrModified', 'DOMNodeInserted', 'DOMNodeRemoved', 'DOMCharacterDataModified',
					'DOMNodeInsertedIntodoc', 'DOMNodeRemovedFromdoc', 'DOMSubtreeModified']
			},
				...
				for (var i in eventTypes) {
					if (eventTypes.hasOwnProperty(i)) {
						if (contains(type, eventTypes[i])) {
							...
							if (i === 'KeyEvents' && ! win.KeyEvent) { i = 'UIEvents'; }
							if (doc.implementation.hasFeature(i, '2.0') || win[i.substring(0, i.length - 1)]) { i = 'Events'; }
